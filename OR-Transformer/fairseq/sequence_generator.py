...
class SequenceGenerator(object):
    ...
    def _generate(
        ...
        trg_lenghts = (target.ne(self.eos) & target.ne(self.pad)).long().sum(
            dim=1) if target is not None else None
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if trg_lenghts is not None:
                self.search.set_src_lengths(trg_lenghts)
            else:
            ...
            if len(finalized_sents) > 0:
                ...
                if trg_lenghts is not None:
                    trg_lenghts = trg_lenghts[batch_idxs]
